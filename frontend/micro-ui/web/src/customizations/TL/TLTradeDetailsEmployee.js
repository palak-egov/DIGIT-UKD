...
const TLTradeDetailsEmployee = ({ config, onSelect, userType, formData, setError, formState, clearErrors }) => {
  ...
  const { isLoading : menuLoading, data: Menu = {} } = window.Digit.Hooks.tl.useTradeLicenseMDMS(stateId, "common-masters", "StructureType");

  const { data: FinaceMenu = {} } = window.Digit.Hooks.tl.useTradeLicenseMDMS(stateId, "egf-master", ["FinancialYear"]);

  const { data: billingSlabData } = window.Digit.Hooks.tl.useTradeLicenseBillingslab({ tenantId, filters: {} });
