...
const TLDocumentsEmployee = ({  config, onSelect, userType, formData, setError: setFormError, clearErrors: clearFormErrors, formState }) => {
  ...
  const { isLoading, data: documentsData } = window.Digit.Hooks.pt.usePropertyMDMS(stateId, "TradeLicense", ["documentObj"]);
      ...
      if (file) {
        if (file.size >= 5242880) {
          ...
          try {
            ...
            const response = await window.Digit.UploadServices.Filestorage("TL", file, tenantId?.split(".")[0]);
