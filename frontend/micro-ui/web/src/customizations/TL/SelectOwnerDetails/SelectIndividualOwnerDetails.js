...
const SelectOwnerDetails = ({ t, config, onSelect, userType, formData }) => {
  ...
  const { data: Menu } = window.Digit.Hooks.tl.useTLGenderMDMS(
    stateId,
    "common-masters",
    "GenderType"
  );
  ...
  function setRelationshipValue(i, value) {
    ...
    if (
      units[i].gender &&
      units[i].mobilenumber &&
      units[i].name &&
      units[i].relationship
    ) {
      setCanmovenext(false);
    }
  }
      ...
      {fields.map((field, index) => {
        return (
          <div key={`${field}-${index}`}>
            ...
            >
              <CardLabel
                style={ismultiple ? { marginBottom: "-15px" } : {}}
              >{`${t("TL_NEW_OWNER_DETAILS_NAME_LABEL")}`}</CardLabel>
              {ismultiple && (
                <LinkButton
                  label={
                    <div>
              ...
              />
              <CardLabel>{`${t("TL_NEW_OWNER_DETAILS_PANCARD")}`}</CardLabel>
              <TextInput
                style={ismultiple ? { background: "#FAFAFA" } : {}}
                t={t}
                type={"text"}
              ...
              <CardLabel>{`${t(
                "TL_NEW_OWNER_DETAILS_GENDER_LABEL"
              )}`}</CardLabel>
              <RadioButtons
                t={t}
                options={TLmenu}
