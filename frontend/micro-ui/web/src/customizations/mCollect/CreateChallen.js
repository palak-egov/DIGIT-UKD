...
const CreateChallen = ({ ChallanData }) => {
  ...
  const { data: fetchBillData } = ChallanData
    ? window.Digit.Hooks.useFetchBillsForBuissnessService(
        {
  ...
  const tenantId = window.window.Digit.SessionStorage.get("Employee.tenantId");
  ...
  useEffect(() => {
    window.Digit.MDMSService.getPaymentRules(tenantId, "[?(@.type=='Adhoc')]").then((value) => {
      setAPIcategories(
