...
export const CollectPayment = (props) => {
  ...
  const { workflow: ModuleWorkflow } = window.Digit.Hooks.useQueryParams();
  ...
  const { data: paymentdetails, isLoading } = window.Digit.Hooks.useFetchPayment({ tenantId: tenantId, consumerCode, businessService });
  ...
  const { posConfig } = usePOSDetails(props, t);
  ...
  const onSubmit = async (data) => {
    ...
    if (data.paymentModeDetails) {
      ...
      if (data.errorMsg) setToast({ key: "error", action: t(data.errorMsg) });
    ...
    try {
      ...
      const resposne = await window.Digit.PaymentService.createReciept(tenantId, recieptRequest);
