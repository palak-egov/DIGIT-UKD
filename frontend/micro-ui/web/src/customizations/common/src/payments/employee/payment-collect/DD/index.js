...
export const useDDDetails = (props, t) => {
  const config = [
    {
      head: t("PAYMENT_DD_HEAD"),
      headId: "paymentInfo",
      body: [
        {
          withoutLabel: true,
          ...
          populators: {
            ...
            component: (props, customProps) => <DDDetailsComponent onChange={props.onChange} ddDetails={props.value} {...customProps} />,
          },
        },
      ],
    },
...
export const DDDetailsComponent = (props) => {
  const { t } = useTranslation();
  const [instrumentDate, setDDDate] = useState(props.ddDetails.instrumentDate);
  const [instrumentNumber, setDDNo] = useState(props.ddDetails.instrumentNumber);
  const [ifscCode, setIfsc] = useState(props.ddDetails.ifscCode);
  const [ifscCodeError, setIfscCodeError] = useState("");
  const [bankName, setBankName] = useState(props.ddDetails.bankName);
  const [bankBranch, setBankBranch] = useState(props.ddDetails.bankBranch?.replace("┬á", " "));
  useEffect(() => {
    if (props.onChange) {
      let errorObj = {};
  ...
  return (
    <React.Fragment>
      <div className="label-field-pair">
        <h2 className="card-label">{t("PAYMENT_DD_NO_LABEL")} *</h2>
        <div className="field">
      ...
      <div className="label-field-pair">
        <h2 className="card-label">{t("PAYMENT_DD_DATE_LABEL")} *</h2>
        <div className="field">
          <div className="field-container">
            <DatePicker
              isRequired={true}
