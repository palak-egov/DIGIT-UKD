    ...
    fetch(`https://ifsc.razorpay.com/${ifscCode}`)
      ...
      .then((payload) => {
        if (payload === "Not Found") {
          ...
          dispatch(prepareFinalObject("validIfscCode", ifscCode));
          dispatch(toggleSpinner());
        }
      })
      .catch((error) => {
  ...
  paidBy: getSelectField({
    ...
    beforeFieldChange: (action, state, dispatch) => {
      ...
      let tabs = get(
        ...
        "pay.components.div.children.formwizardFirstStep.children.paymentDetails.children.cardContent.children.capturePaymentDetails.children.cardContent.children.tabSection.props.tabs",
        []
      );
      let tabValue = get(tabs[tabIndex], "code", "").toLowerCase();
      let componentPath =
        process.env.REACT_APP_NAME === "Citizen"
          ? "components.div.children.formwizardFirstStep.children.paymentDetails.children.cardContent.children.capturePayerDetails.children.cardContent.children.payerDetailsCardContainer"
          : `components.div.children.formwizardFirstStep.children.paymentDetails.children.cardContent.children.capturePaymentDetails.children.cardContent.children.tabSection.props.tabs[${tabIndex}].tabContent.${tabValue}.children.payeeDetails`;
      if (action.value === "COMMON_OTHER") {
        ...
        dispatch(
          handleField(
            ...
            get(
              state,
              "screenConfiguration.preparedFinalObject.ReceiptTemp[0].Bill[0].payerName",
              ""
            )
          )
        );
        dispatch(
          handleField(
            ...
            get(
              state,
              "screenConfiguration.preparedFinalObject.ReceiptTemp[0].Bill[0].mobileNumber",
              ""
            )
          )
        );
